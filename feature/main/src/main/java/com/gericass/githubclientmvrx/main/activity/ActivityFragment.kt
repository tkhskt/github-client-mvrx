...
import com.gericass.githubclientmvrx.common.extensions.toast
...
class ActivityFragment : BaseFragment() {
    ...
    override fun epoxyController() = simpleController(viewModel, mainViewModel) { state, activityState ->
        if (state.eventRequest is Loading) return@simpleController
        activityState.user?.let {
            viewModel.fetch(it.login)
        }
        toast(state.events.toString())
    }
}
